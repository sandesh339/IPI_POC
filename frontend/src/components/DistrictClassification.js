...
const DistrictClassification = ({ data, mapOnly = false, chartOnly = false }) => {
  ...
  const addMapLegend = () => {
    ...
    const existingLegend = document.querySelector('.mapbox-legend-control');
    if (existingLegend) {
    ...
    legendEl.className = 'mapboxgl-ctrl mapboxgl-ctrl-group mapbox-legend-control';
    ...
    if (data.indicator_info) {
      const direction = data.indicator_info.higher_is_better ? 'Higher is better' : 'Lower is better';
      legendContent += `
        <div style="
          margin-top: 8px; 
          padding-top: 6px; 
  ...
  const renderClassificationMap = () => {
        ...
        .map(boundary => {
          ...
          return {
            ...
            properties: {
              ...
              indicator_value: classification.indicator_value,
    ...
    const layerId = 'classification-layer';
    const sourceId = 'classification-source';
    ...
    // Add border layer
    const borderLayerId = 'classification-border';
    ...
    map.current.on('mouseenter', layerId, (e) => {
      ...
      const popupContent = `
        <div class="bg-white p-3 rounded-lg shadow-lg max-w-xs">
          ...
          <p class="text-sm text-gray-600 mb-1"><strong>${props.indicator_name}:</strong> ${parseFloat(props.indicator_value).toFixed(1)}%</p>
  ...
  const renderLegend = () => {
    ...
    return (
      <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl p-6 shadow-xl border border-blue-100">
          ...
          {data.classification_legend.map((item, index) => (
            <div key={index} className="flex items-center p-3 bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200">
              ...
              <div className="flex-1">
                ...
                <div className="text-sm text-blue-600 font-medium">{item.range_text}</div>
          ...
          <div className="mt-6 pt-4 border-t border-blue-200">
            <div className="bg-white rounded-lg p-3 shadow-sm">
              <p className="text-sm text-gray-700 mb-2">
                <strong className="text-blue-600">Indicator Direction:</strong>{' '}
                <span className="font-medium">
              {data.indicator_info.higher_is_better ? 'Higher values are better' : 'Lower values are better'}
                </span>
              </p>
              <p className="text-sm text-gray-600">
                <strong className="text-blue-600">Classification method:</strong> Jenks Natural Breaks
              </p>
            </div>
          </div>
        )}
    ...
    return (
      <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl p-6 shadow-xl border border-green-100">
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          ...
          <div className="bg-white rounded-lg p-4 shadow-sm border border-green-100">
            <p className="text-sm text-green-600 font-medium">Mean Value</p>
            <p className="font-bold text-2xl text-gray-800">{stats.mean_value.toFixed(1)}%</p>
          </div>
          <div className="bg-white rounded-lg p-4 shadow-sm border border-green-100">
            <p className="text-sm text-green-600 font-medium">Range</p>
            <p className="font-bold text-lg text-gray-800">{stats.min_value.toFixed(1)}% - {stats.max_value.toFixed(1)}%</p>
          </div>
          <div className="bg-white rounded-lg p-4 shadow-sm border border-green-100">
            <p className="text-sm text-green-600 font-medium">Std. Deviation</p>
            <p className="font-bold text-2xl text-gray-800">{stats.std_value.toFixed(1)}%</p>
          </div>
        </div>
          ...
          {Object.entries(stats.class_counts).map(([classNum, count]) => {
            ...
            return (
                ...
                <span className="flex items-center">
                  ...
                  <span className="font-semibold text-green-600">{count} ({percentage}%)</span>
                </div>
              );
            })}
          </div>
  ...
  if (!data) {
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-gray-500">No classification data available</div>
      </div>
    );
  }
      ...
      <div className="w-full h-full">
        <div className="bg-white rounded-lg shadow-lg p-4 h-full">
          <h3 className="font-bold text-lg mb-4">
            District Classification Map: {data.indicator_info?.indicator_name}
          </h3>
          <div 
            ref={mapContainer} 
        ...
        <div className="w-full max-w-5xl">
          ...
          <div className="bg-gradient-to-br from-slate-50 to-blue-50 rounded-2xl shadow-2xl p-8 border border-blue-100">
            <h3 className="font-bold text-2xl mb-6 text-gray-800 text-center bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">
              District Count by Performance Class: {data.indicator_info?.indicator_name}
            </h3>
    ...
    <div className="w-full space-y-6">
      ...
      {data.indicator_info && (
        <div className="bg-gradient-to-r from-blue-600 to-blue-800 text-white p-6 rounded-lg">
          <h2 className="text-2xl font-bold mb-2">
            District Classification: {data.indicator_info.indicator_name}
          </h2>
          <p className="text-blue-100">
            {data.statistics?.total_districts} districts classified into 4 performance classes using Jenks natural breaks
          </p>
          {data.state_filter && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2">
          <div className="bg-white rounded-lg shadow-lg p-4">
            <h3 className="font-bold text-lg mb-4">Classification Map</h3>
            <div 
              ref={mapContainer} 
      ...
      <div className="flex justify-center">
        <div className="w-full max-w-4xl">
          <div className="bg-gradient-to-br from-slate-50 to-blue-50 rounded-2xl shadow-2xl p-8 border border-blue-100">
            <h3 className="font-bold text-2xl mb-6 text-gray-800 text-center bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">
              District Count by Performance Class
            </h3>
...
export default DistrictClassification;
